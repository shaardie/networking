...
CONTAINERlab can do many things more and you can spin up complex environment even with different network operating systems, so if you want to know more about that, check it out yourself.
...
You can see the actual Dockerfile [here](./Dockerfile).

There are some networking tools, we will use quite often which are not part of the setup itself, but helps us analyse and *see* what is happening in the network, like [tcpdump](https://www.tcpdump.org/), which can be use to actually look at the network traffic.
It is a very powerful tool and we will familiarize ourselves with some, but not all, of its functionality along the way.

Despite that, we sometimes use [ipcalc](https://jodies.de/ipcalc) to analyse ip addresses and networks.

## Networking Basics

Before we dive into the different setups, we need to talk a little bit about the basics of networking.

So if we talk about networking we want to connect multiple computers or devices together so that they can share information.
How this information is shared between different computers, we also often say nodes, is the [OSI Model](https://en.wikipedia.org/wiki/OSI_model).

Since there are a lot of different technologies and protocols involved during the communication, it is quite difficult to always think about all those at once.
...
Otherwise read about it, if you want.
...
The *physical layer* or *layer 1* is responsible for the actual transmission and reception of the raw data.
Think for example about the hardware of your network.
We will not really talk about this that much, but we will describe the *physical* connections between our nodes, so I needed to at least mention this.
...
So if we later take a look at communication between two nodes, we look at communication between two of those addresses.
...
We have two different kinds of IP addresses, `IPv4` and `IPv6`.
...
This describes the IPv4 Address `192.168.1.1` as part of the network `192.168.1.0/24`, which consist of all ip addresses where the last entry of the address changes.
...
* [Direct Connection](/direct-connection/Readme.md): The simplest of the setups. Just a direct connection between two nodes.
* Layer 2 Basics:
